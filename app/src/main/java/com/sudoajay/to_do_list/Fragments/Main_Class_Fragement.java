...
public class Main_Class_Fragement extends Fragment implements View.OnClickListener {
    ...
    private void Grab_The_Data_From_DB() {
        ...
        if (!mainDataBase.check_For_Empty()) {
            Cursor cursor = mainDataBase.GetAllData();
            if (cursor != null) {
                cursor.moveToFirst();
                do {
    ...
    private void Suitable_Date_From_Array() {
        ...
        for (int i = 0; i < save_All_Date.size(); i++) {
            Date curDate = null;
            try {
                curDate = format.parse(save_All_Date.get(i));
            } catch (ParseException e) {
                e.printStackTrace();
            }

            assert curDate != null;
            if (curDate.before(todayDate)) {
    ...
    private void createNewData(int i) {
        ...
        if (cursor != null) {
            ...
            DateFormat format = new SimpleDateFormat("dd-MM-yyyy", Locale.ENGLISH);
            try {
                String nextDate = Next_Day(count, cursor.getString(2), i);
                Date curDate = format.parse(nextDate);
                if (curDate.before(todayDate)) {
                    Cursor cursor1 = mainDataBase.GetTheId(cursor.getString(1), nextDate);
                    if (cursor1 != null) {
                        cursor1.moveToFirst();
                        do {
                            array_Id.add(cursor1.getInt(1));
                            save_All_Date.add(cursor1.getString(0));
                            if (cursor1.getInt(2) == 0)
                                check_Box_Array.add(false);
                            else {
                                check_Box_Array.add(true);
                            }
                            onRepeatId.add(cursor1.getInt(3));

                            // if its have custom weekdays
                            if (cursor1.getInt(3) == 5) weeks.add(cursor1.getString(4));
                            else {
                                weeks.add(null);
                            }
                            endlesslyData.add(cursor1.getString(5));
